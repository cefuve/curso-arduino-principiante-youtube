/*
Puse el comentario "added" a las líneas
que agregué para implementar el almacenamiento
del turno actual en la EEPROM.
...
void setup() {
  ...
  //Si es la primera vez que se graba un valor en
  //la EEPROM, se sugiere que se escriba un valor
  //incial con la función write.
  
  //EEPROM.write(direccion, 0);
  count = EEPROM.read(direccion);   //added
}
...
void loop() {
  ...
  if(lectura == 0 && estado_anterior == 1){
    ...
    EEPROM.update(direccion, count);    //added
  }
